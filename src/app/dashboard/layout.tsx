        ...
        <Fragment>
            <div className="flex bg-[var(--background)]">
                ...
                <div className={`
                    ...
                    w-72 bg-[var(--card-bg)] z-30
                    border-r border-[var(--card-border)] shadow-[var(--card-shadow)]
                    flex flex-col
                `}>
                    {renderHeader()}

                    {/* Collapse Toggle Button - Desktop Only - Moved to top */}
                    <div className="hidden lg:block absolute -right-3 top-4 z-40">
                        <button
                            onClick={toggleSidebarCollapse}
                            className="p-2 rounded-full bg-[var(--card-bg)] hover:bg-[var(--hover-bg)] transition-colors duration-200 border border-[var(--card-border)] shadow-[var(--card-shadow)]"
                            aria-label={isSidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}
                        >
                            <IoIosArrowForward
                                className={`w-5 h-5 text-[var(--text-secondary)] transition-transform duration-300 ${isSidebarCollapsed ? 'rotate-0' : 'rotate-180'}`}
                            />
                        </button>
                    </div>
                </div>
                ...
                {isSidebarOpen && (
                    <div
                        className="fixed inset-0 bg-[var(--text)]/60 backdrop-blur-sm lg:hidden z-20 animate-fade-in"
                        onClick={() => setIsSidebarOpen(false)}
                    />
                )}

                {/* Main Content */}
                <div className={`flex-1 relative w-full transition-all duration-300 ${isSidebarCollapsed ? 'lg:w-[calc(100%-80px)]' : 'lg:w-[calc(100%-280px)]'}`}>
                    {/* Top Navigation Bar */}
                    <div className="sticky top-0 z-20 h-16 bg-[var(--header-bg)] border-b border-[var(--header-border)] flex items-center justify-between px-4 lg:px-6 backdrop-blur-sm shadow-[var(--header-shadow)]">
                        {/* Left side */}
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {/* Theme Toggle */}
                            <ThemeToggle onClick={() => setIsThemeModalOpen(true)} />
                        </div>
                    </div>
