  ...
  useEffect(() => {
    const fetchChartData = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "Using fallback query. Please create the suggested index."
          );
    ...
    const processTransactions = (snapshot: QuerySnapshot<DocumentData>) => {
      ...
      transactions.forEach((transaction) => {
        ...
        salesByDay.set(date, (salesByDay.get(date) || 0) + transaction.amount);
      });
      ...
      transactions.forEach((transaction) => {
        ...
        licenseCount.set(licenseType, (licenseCount.get(licenseType) || 0) + 1);
      });

      const colors = ["#f43f5e", "#0ea5e9", "#8b5cf6", "#10b981"];

      // Process top items
      const topItems: TopSellingItem[] = transactions
        .slice(0, 3)
        .map((transaction) => ({
