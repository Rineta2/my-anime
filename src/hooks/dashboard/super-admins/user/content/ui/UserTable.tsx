...
import { formatPhoneNumberForDisplay } from '@/hooks/dashboard/super-admins/user/utils/formatters'
            ...
            <div className="overflow-x-auto custom-scrollbar">
                <table className="min-w-full divide-y divide-gray-200/80">
                    <thead className="bg-[var(--card-bg)] backdrop-blur-sm">
                        <tr>
                            <th className="px-4 sm:px-6 py-5 text-left text-xs sm:text-sm font-semibold tracking-wider">Name</th>
                            <th className="px-4 sm:px-6 py-5 text-left text-xs sm:text-sm font-semibold tracking-wider">Email</th>
                            <th className="hidden lg:table-cell px-4 sm:px-6 py-5 text-left text-xs sm:text-sm font-semibold tracking-wider">Phone</th>
                            <th className="px-4 sm:px-6 py-5 text-left text-xs sm:text-sm font-semibold tracking-wider">Status</th>
                            <th className="px-4 sm:px-6 py-5 text-xs sm:text-sm font-semibold tracking-wider">Actions</th>
                        </tr>
                    </thead>
                    <motion.tbody
    ...
    return (
        ...
        >
            <td className="px-4 sm:px-6 py-5 whitespace-nowrap">
                <div className="flex items-center gap-3 sm:gap-4">
                    {user.photoURL ? (
                        ...
                        <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-2xl flex items-center justify-center shadow-sm border border-gray-100/80">
                            <span className=" text-base sm:text-lg font-medium">{user.displayName?.[0]}</span>
                        </div>
                    )}
                    <div>
                        <div className="text-sm sm:text-base font-medium">{user.displayName}</div>
                        <div className="text-xs lg:hidden">{user.email}</div>
                    </div>
                </div>
            </td>
            ...
            <td className="hidden lg:table-cell px-4 sm:px-6 py-5 whitespace-nowrap">
                <span className="text-sm">{formatPhoneNumberForDisplay(user.phoneNumber || '')}</span>
            </td>
