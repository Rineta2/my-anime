...
import TransactionPaidDetails from '@/hooks/dashboard/super-admins/transaction/paid/TransactionPaidDetails'

import { Transaction } from '@/hooks/dashboard/super-admins/transaction/transaction/types/transaction'

import TransactionPaidSkeleton from "@/hooks/dashboard/super-admins/transaction/transaction/TransactionSkeleton"
...
export default function TransactionLayout() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(transactionsRef, (querySnapshot) => {
      ...
      // Filter transactions with success status
      const successTransactions = transactionsData.filter(transaction => transaction.status === 'success')
    ...
    return (
      ...
      >
        ...
        <p className="text-center max-w-md">
          There are currently no paid transactions in the system. Check back later for updates.
        </p>
      </motion.div>
    )
  }
    ...
    <section className='min-h-screen'>
      ...
      >
        ...
        >
          ...
          >
            Daftar transaksi yang berhasil diproses
          </motion.p>
        </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {transactions.map((transaction) => (
          <TransactionPaidDetails key={transaction.id} transaction={transaction} />
        ))}
      </div>
    </section>
  )
