...
import TransactionCancelledDetails from '@/hooks/dashboard/super-admins/transaction/cancelled/TransactionCancelledDetails'
...
export default function TransactionLayout() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(transactionsRef, (querySnapshot) => {
      ...
      // Filter transactions with cancelled status
      const cancelledTransactions = transactionsData.filter(transaction => transaction.status === 'cancelled')
    ...
    return (
      ...
      >
        ...
        <p className="text-center max-w-md">
          There are currently no cancelled transactions in the system. Check back later for updates.
        </p>
      </motion.div>
    )
  }
    ...
    <section className='min-h-screen'>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {transactions.map((transaction) => (
          <TransactionCancelledDetails key={transaction.id} transaction={transaction} />
        ))}
      </div>
    </section>
  )
