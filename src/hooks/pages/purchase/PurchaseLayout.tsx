      ...
      <div className="container px-4 md:px-10">
        <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8'>
          {user && (
            <div className="w-full md:w-auto">
              <div className="flex items-center space-x-3 p-3 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 shadow-lg hover:shadow-xl transition-all duration-300">
                ...
                <div>
                  <h2 className="text-base font-semibold text-white">{user.displayName}</h2>
          ...
          <div className='w-full md:w-auto flex items-center gap-2 rounded-xl bg-white/5 backdrop-blur-sm p-2.5 border border-white/10 shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer group'>
            <IoTicket className="text-lg text-purple-400 group-hover:text-purple-300 transition-colors" />
            <h2 className="text-sm font-medium text-white group-hover:text-purple-300 transition-colors">Tukar Voucher</h2>
          </div>
        </div>

        <div className='rounded-xl bg-white/5 backdrop-blur-sm p-5 border border-white/10 shadow-lg mb-8'>
          <p className="mb-3 text-gray-300 text-sm">Langganan Premium untuk menikmati ribuan episode secara gratis</p>
          <h3 className="text-xl font-bold mb-4 text-white">Keuntungan Premium</h3>

          <div className='flex flex-wrap gap-3'>
            {dataCard.map((item) => (
              <div key={item.id} className="flex items-center gap-2.5 p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition-all duration-300 group flex-1 min-w-[180px]">
                <div className="p-1.5 bg-purple-500/10 rounded-md group-hover:bg-purple-500/20 transition-colors">
                  {item.icons}
                </div>
                <h2 className="text-sm text-white group-hover:text-purple-300 transition-colors">{item.name}</h2>
              </div>
            ))}
          </div>
        </div>

        <h1 className="text-2xl font-bold mb-6 text-white">Pilihan Paket Premium</h1>

        <div className="flex flex-wrap gap-4">
          {prices.map((price) => (
            <div
              key={price.id}
              className={`bg-white/5 backdrop-blur-sm rounded-xl p-5 border transition-all duration-300 hover:shadow-xl flex-1 min-w-[260px] ${selectedPrice === price.id
                ? 'border-purple-500 shadow-lg shadow-purple-500/20'
                : 'border-white/10 hover:border-purple-500/50'
                } cursor-pointer`}
              onClick={() => handlePriceSelect(price.id)}
            >
              <h2 className="text-lg font-semibold mb-3 text-white">{price.title}</h2>

              <div className='flex flex-col gap-3'>
                {price.discount && (
                  <div className="flex flex-col gap-2 space-y-3">
                    <span className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-4 py-2 rounded-lg text-lg font-medium w-fit shadow-lg shadow-purple-500/20">
                      Rp. {Number(price.discount).toLocaleString('id-ID')}
                    </span>

                    <div className='flex items-center gap-2'>
                      {price.originalPrice && (
                        <span className="text-gray-400 line-through text-sm">Rp. {Number(price.originalPrice).toLocaleString('id-ID')}</span>
                      )}
        ...
        <div className="mt-6 flex justify-center">
          <button
            ...
            className={`px-6 py-3 rounded-lg text-white font-medium transition-all duration-300 ${selectedPrice && selectedCard
