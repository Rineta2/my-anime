...
import { SearchModalProps } from '@/components/ui/article/types/article';
  ...
  return (
    ...
    >
      <motion.div
        ...
        exit={{ opacity: 0, y: -50 }}
        className="modal-box max-w-3xl bg-[var(--card-bg)] border border-[var(--border-color)]"
      >
        <div className="relative">
          <input
            ...
            className="w-full pl-14 pr-6 py-4 text-lg bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl
                                 border-none focus:outline-none focus:ring-2 focus:ring-blue-500/20
                                 transition-all duration-300 placeholder:text-gray-400"
            autoFocus
          />
          <svg className="h-6 w-6 absolute left-5 top-1/2 transform -translate-y-1/2"
        ...
        {searchResults.length > 0 && (
          <div className='mt-8'>
            <h3 className="font-medium mb-4 px-2">Search Results</h3>
            <div className="space-y-4 max-h-[60vh] overflow-y-auto px-2 custom-scrollbar">
              {searchResults.map((article) => (
                <div
                  key={article.id}
                  onClick={() => handleSearchResultClick(article)}
                  className="flex items-start gap-4 p-4 rounded-2xl hover:bg-gray-700 cursor-pointer transition-all duration-300 group bg-[var(--card-bg)] border border-[var(--border-color)]"
                >
